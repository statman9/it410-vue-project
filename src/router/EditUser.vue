<template>
    <div id="app" :class="{ loaded: loaded }">
        <form id="editform" @submit.prevent="getOtherData" method="POST" :action="'/api/users/'+ player.username" v-if="user && (user.usertype === 'admin' || user.usertype === 'heacoach')">
            ...
            <p>Permissions: <select v-model="player.usertype" name="usertype" class="form-control">
                <option v-for="permission in permissions" v-bind:key="permission.Name" :value="permission.Value">
                    {{permission.Name}}
                </option>    
            </select></p>
            <p v-if="player.usertype === 'player' || player.usertype === 'recruit'">Position: <select v-model="player.position" name="position" class="form-control">
            ...
            <p style="display: none;"><input type="text" :value="JSON.stringify(player.messages)" name="messages" /></p>
            <p style="display: none;"><input type="text" :value="JSON.stringify(player.depthChartPosition)" name="depthChartPosition" /></p>
